...
class Fabits:
    def __init__(self)->None:
        ...
        self.admnu(); self.adcon(); self.icc(self.cfg.get('ani',1),new=1)
        self.rt.config(menu=self.mnu); self.rt.mainloop()
    def adcon(self)->None:
        ...
        slb=tkinter.Scrollbar(self.memp1); self.ls.column('#0',width=0,stretch=0)
        self.ls.column('opt',width=30*self.scr,anchor='w')
        self.ls.config(yscrollcommand=slb.set); slb.config(command=self.ls.yview)
        self.memp2=ttk.Frame(self.rt); self.tetr=tkinter.Text(self.memp2,undo=1)
        ...
        self.ofl=self.nfl=self.chg=0; self.txflnm='未命名文件'; self.clear(self.ls)
        slb.pack(side='right',fill='y'); self.ls.pack(fill='both',expand=1)
        slb2.pack(side='right',fill='y'); self.tetr.pack(fill='both',expand=1)
    ...
    def adfun(self,lbl:str,knd:dict[str:any])->None:
    ...
    def calc(self)->None:
        ...
        self.calshw,caltx=[ttk.Label]*2,[' ','I']; self.res=self.m=0.0; self.acc=12
        calcmp=[ttk.Frame(cal) for i in range(8)]
        self.expsyn,self.lexp,self.expidx=['']*100,0,0
        for i in range(2):
        ...
        for i in range(6):
            for j in range(7):
                ...
                calbtns.pack(side='left',fill='both',expand=1)
        self.empck(calcmp)
    def cald(self,syn:list[str],lsyn:int)->float:
    ...
    def calsyn(self,expsyn:list[float|str|tuple])->float|tuple:
        lexp=len(expsyn); tl,rl,tsyn,rsyn,stc=lexp,0,[any]*100,[any]*100,1
        for i in range(lexp):
            tsyn[i]=expsyn[i]
            if expsyn[i]==',':
        ...
        while stc:
            ...
            tl,fls,stc=0,0,0
            for i in range(rl):
                if fls: fls=0; continue
                if i<rl-1 and rsyn[i+1]=='!':
                    if isinstance(rsyn[i],float):
                        tsyn[tl]=math.gamma(rsyn[i]+1); tl+=1; stc=fls=1
                    else: tsyn[tl]=rsyn[i]; tl+=1
                elif rsyn[i] in self.funs:
                    if isinstance(rsyn[i+1],float):
                        tsyn[tl]=self.funs[rsyn[i]](rsyn[i+1]); tl+=1; stc=fls=1
                    elif isinstance(rsyn[i+1],tuple):
                        tsyn[tl]=self.funs[rsyn[i]](rsyn[i+1][0],rsyn[i+1][1]); tl+=1; stc=fls=1
                    else: tsyn[tl]=rsyn[i]; tl+=1
                else: tsyn[tl]=rsyn[i]; tl+=1
        for i in self.bas:
            j,fls,rl=0,0,0
    ...
    def cdmix(self,fls:list[str])->None:
    ...
    def check(self)->None:
        ...
        self.bgidx,self.fnt,self.fwgt=self.cfg.get('bgidx',-1),self.cfg.get('font',''),''
    ...
    def clrpld(self,pnm:str,nclr:list[tuple[int,int,int]],clr:tuple[int,int,int])->None:
    ...
    def cmdel(self)->None:
        ...
        if self.mb('q','yn','','确认删除?')=='yes':
            ...
            self.cmdtr.delete(scl); self.cfg['commands'].pop(lnm)
    def cmdinp(self,st:str)->str|None:
        ...
        cinlb.pack(expand=1); cintr.pack(expand=1); self.empck(cinemp)
    ...
    def cmdmng(self)->None:
        ...
        cmds=[self.cmdad,self.cmdmd,self.cmdel,lambda: self.cmdrn(pre=1),
              self.cmdrn,lambda: self.winqut(cmd,'cmdmng')]
        ...
        cmdslb.pack(side='right',fill='y'); self.cmdtr.pack(fill='both',expand=1)
        for i in range(6):
        ...
        self.cmdres.column('#0',width=0,stretch=0); cmdsr=tkinter.Scrollbar(cmdemp[2])
        ...
        cmdsr.pack(side='right',fill='y'); self.cmdres.pack(fill='both',expand=1)
        self.clear(self.cmdres)
        if 'commands' not in self.cfg: self.cfg['commands']={}
        for i in self.cfg['commands']: self.cmupd(self.cmdtr,i,self.cfg['commands'][i],'green')
        self.empck(cmdemp)
    def cmdp(self,cmd:str,tag:ttk.Treeview)->None:
        pcs=subprocess.Popen(cmd,shell=1,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=1)
    ...
    def cmdrn(self,pre=0)->None:
        scl=self.cmdtr.selection()
        if not scl: return
        nm,cmd=self.cmdtr.item(scl,'values'); exc,arg,args,flg='','',[],0
        for i in cmd:
            if flg:
                if i=='}': flg=0; exc+=i; args+=[arg]; arg=''
                else: arg+=i
            else:
                exc+=i
                if i=='{': flg=1
    ...
    def cmpil(self)->None:
        ...
        self.cmptr.column('#0',width=0,stretch=0); cmpslb=tkinter.Scrollbar(cemp[3])
        ...
        cmpslb.pack(side='right',fill='y'); self.cmptr.pack(fill='both',expand=1)
    ...
    def cmprt(self,st:str,tag:ttk.Treeview,cl:str)->None:
        i=0; k=st[i:i+45]
        while k: self.show(tag,k,cl); i+=45; k=st[i:i+45]
    def conpuw(self)->None:
        ...
        contx=['祈愿一次','祈愿十次','退出']; self.conbtn=[ttk.Button]*3
        concmd=[lambda: self.pul(1),lambda: self.pul(10),lambda: self.winqut(puw,'conpuw')]
        puwlb=ttk.Label(pu[0],text='祈愿结果'); puwlb.pack(expand=1)
        self.et=ttk.Treeview(pu[1],columns=('opt',),show='tree')
        self.et.column('#0',width=0,stretch=0); slet=tkinter.Scrollbar(pu[1])
        ...
        self.et.pack(fill='both',expand=1); self.empck(pu)
    def crttpl(self,tle:str,w:int,h:int,wid:str,re:int)->tkinter.Toplevel:
    ...
    @staticmethod
    def dlg(n:int,tle:str,flt:tuple[str,str])->str:
    ...
    def genchk(self)->None:
        arg=[0]*6
        try:
            for i in range(6): arg[i]=int(self.mzvars[i].get())
            self.memp1.pack_forget(); self.cvs.pack(fill='both',expand=1)
    ...
    def getvar(self,tag:tkinter.Toplevel,arg:str,knd:type)->None:
        res=self.var.get()
        try:
    ...
    def hlp(self)->None:
        ...
        hlpslb=tkinter.Scrollbar(hlp); self.hlptrv.column('#0',width=0,stretch=0)
        ...
        hlpslb.pack(side='right',fill='y'); self.hlptrv.pack(fill='both',expand=1)
        self.hlpshw('F')
    def hlpshw(self,k:str)->None:
    ...
    def icc(self,show:int,new=0)->None:
        if not show: self.rt.after(0,self.ice,new); return
        self.memp1.pack_forget(); sztmp=lambda x: round(x*self.scr)
        self.restul(); self.cvs.pack(fill='both',expand=1)
        self.ico(self.data['icc'],60,sztmp(0.5),iter('d'))
        self.rt.after(500,self.icd,sztmp,new)
    def icd(self,sztmp,new:int)->None:
    ...
    def imgsrt(self)->None:
        ...
        lnr,cct,lpr=range(cbbln),multiprocessing.cpu_count(),0
        res,pol=[[None,0,''] for i in lnr],multiprocessing.Pool(processes=cct)
        self.rt.after(0,self.pginit,'图片排序',cbbln); self.cbbcnt=0
        calbk=lambda *args: self.cbbpgu(cbbln)
        for i in lnr:
            nm=self.pnm(names[i])
            tsk=pol.apply_async(avgs,args=[nm],callback=calbk)
            res[lpr][0]=tsk; res[lpr][2]=nm; lpr+=1
        pol.close(); pol.join(); self.rt.after(1000,self.winqut,self.pgm,'pginit')
        for i in lnr: res[i][1]=res[i][0].get()
        res=sorted(res,key=lambda i:i[1])
        self.rt.after(0,self.show,self.ls,'(3/3)整理','cyan')
        for i in lnr: os.rename(self.pnm(res[i][2]),self.pnm(f'pix{i:04d}.png'))
        names=[i for i in os.listdir(self.pth) if i.lower().endswith('.png')]
        for i in lnr: os.rename(self.pnm(names[i]),self.pnm(f'pic{i:04d}.png'))
        self.rt.after(0,self.show,self.ls,'进程已结束','red')
        self.rt.after(0,self.show,self.ls,'>>>','purple')
    def inp(self,st:str,knd=str,show='')->any:
        ...
        inlb.pack(expand=1); etr.pack(expand=1); self.empck(inemp)
    ...
    def inperr(self,tag:tkinter.Toplevel,arg:str,val=None)->None:
        self.reval=val; self.winqut(tag,arg)
    def iso(self)->None:
        n=self.inp('基团-CnH2n+1,输入n值',int)
        if n is None: return
        self.thr(lambda: self.isocal(n))
    def isocal(self,n:int)->None:
    ...
    def itsth(self)->None:
        ...
        ittx=['获取','强化','退出']; self.itbtn=[ttk.Button]*3
        itcmd=[self.getit,self.upgd,lambda: self.winqut(itw,'itsth')]
        itlb=ttk.Label(it[0],text='强化结果:'); itlb.pack(expand=1)
        self.et=ttk.Treeview(it[1],columns=('opt',),show='tree')
        self.et.column('#0',width=0,stretch=0); slet=tkinter.Scrollbar(it[1])
        self.et.column('opt',width=15*self.scr,anchor='w')
        self.et.config(yscrollcommand=slet.set); slet.config(command=self.et.yview)
        for i in self.data['clr']: self.et.tag_configure(i,foreground=i,background=self.bg)
        for i in range(3):
        ...
        self.et.pack(fill='both',expand=1); self.empck(it)
    def lnkcal(self,ar:str,hd:int)->None:
        lnkl,cur=0,hd; arr=json.loads(ar)
    ...
    def lnksrt(self)->None:
        ...
        self.thr(lambda: self.lnkcal(ar,hd))
    def mazepl(self)->None:
        ...
        self.mzbtns=[ttk.Button]*3; mzbtx=['生成','解','退出']
        ...
        self.mzbtns[1].config(state='disabled'); lb.pack(expand=1); self.empck(mzemp)
    def mb(self,icn:str,tp:str,tle:str,msg:str)->str:
        mbs=self.data['mb']
        mbt=messagebox.Message(icon=mbs[icn],type=mbs[tp],title=tle,message=msg)
        res=mbt.show(); return res
    def mngrd(self,k:int,p:int)->None:
        self.mnget[k][p].config(state='normal'); self.mnget[k][1-p].config(state='disabled')
    def mzrst(self,tag:tkinter.Toplevel)->None:
        self.winqut(tag,'mazepl'); self.cvs.pack_forget(); self.restul()
        self.memp1.pack(side='bottom',fill='both',expand=1)
    def mzshw(self,lx:int,ly:int,rx:int,ry:int,clr:str)->None:
        ...
        self.tl.end_fill()
    def opnf(self,ext=0,nda=0,opn=None)->None:
        self.ofl=1; self.memp2.pack(fill='both',expand=1)
        if self.chg:
    ...
    def pginit(self,tx:str,tol:int)->None:
        ...
        self.pgt.column('#0',width=0,stretch=0)
        ...
        self.pgb['maximum']=tol; self.pgl.pack(expand=1)
        self.pgb.pack(fill='y',expand=1); pgsb.pack(side='right',fill='y')
        self.pgt.pack(fill='both',expand=1); self.empck(pgf)
    def pgu(self,num:int,tx:str,clr:str)->None:
    ...
    def pulpro(self,vals:list[int])->None:
        ...
        for i in range(pb):
            for j in range(_len):
                ...
                ups=min(ups,50); pros=self.data['pro_lst5'][puts]
    ...
    def recmd(self,k:int)->None:
        ...
        else: self.thr(lambda: self.renm(show=1))
    def redo(self)->None:
        if self.ofl:
    ...
    def renm(self,show=0)->None:
        ...
        i,flg,form,args,lnms=0,0,'','',len(names)
        while i<lmdl:
            form+=model[i]
            if model[i]=='{':
                for j in keys:
    ...
    def ring(self)->None:
        ...
        if n is None: return
        self.thr(lambda: self.rical(n))
    def rndchr(self,itx:str,n:int)->str:
    ...
    def savcfg(self)->None:
        ...
        if self.reboot:
            ...
            if __file__.endswith('.py'):
                ...
                if self.rbtarg: subprocess.Popen([now,self.rbtarg])
                else: subprocess.Popen([now])
            exit(0)
    def savf(self,at=0,nda=0)->None:
        if not self.ofl: return
        if nda:
        ...
        fl=open(self.txflnm,'w',encoding='utf-8',newline='\n'); self.nfl=self.chg=0
        txres=self.tetr.get('1.0','end'); fl.write(txres); fl.close()
    def sch(self,dire:int,al=0)->None:
        ...
        if al: pos=self.tetr.search(schtx,st,ed,nocase=ncs)
            ...
            pos=self.tetr.search(schtx,cur,st,backwards=1,nocase=ncs)
            if not pos: pos=self.tetr.search(schtx,ed,cur,backwards=1,nocase=ncs)
        if pos:
    ...
    def schgd(self)->None:
        ...
        self.upchk=tkinter.BooleanVar(value=0)
        scs=[ttk.Frame(schtl) for i in range(5)]; txs=['查找','替换为']
        schbtx=['向上查找','向下查找','从头查找','替换','全部替换','退出']
        btncmd=[lambda: self.sch(dire=0),lambda: self.sch(dire=1),
                lambda: self.sch(dire=1,al=1),self.rplc,self.rplcal,
    ...
    def slv(self)->None:
        ...
        stk[top]=[self.bx*2-1,self.by*2-1,0]; flg=0
        while top>=0:
            ...
            if self.maze[x+tmp[i][0]*2][y+tmp[i][1]*2]==2: flg=1; continue
    ...
    def syncal(self,expsyn:list[str])->None:
    ...
    def txcbtb(self,byt:str,opn=0,clos=0,opnfl='')->str|None:
        if opn:
    ...
    def txdel(self,itx:str,new:str,n:int,funnum:int)->None:
    ...
    def txmng(self)->None:
        ...
        self.mnget=[[ttk.Entry]*2,[ttk.Entry]*2]
